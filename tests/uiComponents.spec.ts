...
test.describe('Form Layout page', () =>{
    ...
    //test.describe.configure({mode: 'serial'}) //NOT RECOMENDED TO HAVE DEPENDENCY BETWEEN TESTS
    test.beforeEach(async({page}) =>{
...
test('tooltips', async({page}) => {
    ...
    page.getByRole('tooltip') //if you have a role toltip created
...
test('dialog boxes', async({page}) => {
    ...
    const show = await page.locator("tbody tr").first().textContent()
    //console.log(show)
...
test.describe('tables', ()=> {
    ...
    test('table1', async({page}) => {
        ...
        const targetTow = page.getByRole('row', {name: "twitter@outlook.com"})
        await targetTow.locator('.nb-edit').click()
...
test.describe('datepickers', ()=> {
    ...
    test('datepicker1', async({page}) => {
        const calendarInputFieled = page.getByPlaceholder('Form Picker')
        await calendarInputFieled.click()
        await page.locator('[class="day-cell ng-star-inserted"]').getByText('1', {exact: true}).click()
        await expect(calendarInputFieled).toHaveValue('May 1, 2025')
    })
